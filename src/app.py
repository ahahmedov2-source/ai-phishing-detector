...
class PhishingDetectorApp:
    ...
    def run(self):
        ...
        if not self.config_manager.is_configured():
            ...
            # === ИНСТРУКЦИИ ЗА ПОТРЕБИТЕЛИ (прибиращи се) ===
            with st.expander("📋 ИНСТРУКЦИИ ЗА ПОТРЕБИТЕЛИ - Как да използвате приложението", expanded=False):
                st.markdown("""
### 👋 Добре дошли в AI Детектора за Фишинг Имейли!

Това приложение помага да проверите дали имейлът е **опасен (фишинг)** или безопасен.

### Стъпка по стъпка:


#### 🔹 Начин 1: Paste Headers (препоръчителен)

**1.** Отворете имейла, който искате да проверите (в Gmail, Outlook и др.).

**2.** Натиснете **трите точки ⋮** → **"Show original"** или **"Показване на оригинала"**.

**3.** Копирайте **всичко** от началото (заглавията + съдържанието) или натиснете сният бутон "копирай в клипборд".

**4.** Върнете се тук и поставете копираното в голямото поле по-долу.

**5.** Натиснете бутона **Analyze Email**.


#### 🔹 Начин 2: Upload File
**1.** Запазете имейла като файл (.eml формат).  

**2.** Натиснете **"Upload File"**.  

**3.** Изберете файла или го плъзнете в полето.  

**4.** Натиснете **Analyze Email**.


##Готово! Приложението ще ви каже дали имейлът е безопасен или опасен.
""")
